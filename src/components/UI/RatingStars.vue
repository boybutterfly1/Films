...
const hoverStarHandle = (star?: number) => {
  if (star) {
    hoveredStar.value = star
    rating.value = hoveredStar.value
  } else {
    hoveredStar.value = null
    rating.value = selectedStar.value ? selectedStar.value : null
  }
}
const selectStar = (star: number) => {
  selectedStar.value = star
  rating.value = selectedStar.value
  props.film ? props.film.userRating = rating.value : null;
  if (!filmsStore.ratedFilms.some(film => props.film ? film.id === props.film.id : null)) {
    props.film ? filmsStore.ratedFilms.push(props.film) : null
...
<template>
  <div class="rating">
    <div
      v-for="star in 10"
      :key="star"
      class="rating__stars-line"
      @mouseover="hoverStarHandle(star)"
      @mouseout="hoverStarHandle()"
      @click="selectStar(star)"
    >
      <img
          class="rating__stars-line__img"
          :src="star <= hoveredStar || star <= selectedStar ? activeStar : emptyStar"
          alt="star"
      >
    </div>
